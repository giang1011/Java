...
public class Main {
    public static void main(String[] args) {
        ...
        customerService.sortAccountsByCustomerName(accounts);
        customerService.sortInvoicesByCustomerName(invoices);


        Optional<Account> accountOpt = customerService.findAccountById(accounts, 2);
        accountOpt.ifPresent(account -> System.out.println("Found Account for customer: " + account.getCustomer().getName()));

        System.out.println("\nList of customers eligible for payment:");
        for (Customer customer : customers) {
            customerService.listCustomersAbleToPay(accounts, invoices, customer);
        }

        System.out.println("\nList of customers who are not eligible for payment:");

        customerService.listCustomersUnableToPay(customers, invoices, accounts);

        customerService.applyForFemalesInAugust(invoices);
    }
}