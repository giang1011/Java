...
public class Econtroller {
    ...
    public void sortInvoicesByCustomerName(List<Invoice> invoices) {
        List<Invoice> sortedInvoices = invoices.stream()
                .sorted(Comparator.comparing(invoice -> invoice.getCustomer().getName())) // Sort by customer name
                .collect(Collectors.toList());
            ...
            accounts.stream()
                    ...
                    .forEach(account -> {
                        invoices.stream()
                                ...
                                .forEach(invoice -> {
                                    double amountAfterDiscount = invoice.getAmount() * (1 - (customer.getDiscount() / 100.0));
                                    if (account.getBalance() < amountAfterDiscount) { // Changed to amountAfterDiscount
